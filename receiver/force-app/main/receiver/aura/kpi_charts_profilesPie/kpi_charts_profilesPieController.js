...
({
  doInit: function(cmp, evt, helper) {
    let action = cmp.get('c.getProfilesChartData'); //call new method
    let randomNumber = Math.floor(Math.random() * 100000);
    cmp.set('v.canvasId', 'profilesPieChart' + randomNumber);
    ...
    secondAction.setParams({ whatReport: 'profiles' }); // if we want go to functionality change this as well
  ...
  handleFilters: function(cmp, evt, helper) {
    ...
    let action = cmp.get('c.getProfilesChartData');
    ...
    action.setCallback(this, function(response) {
      ...
      if (state === 'SUCCESS') {
        ...
        if (
          Object.keys(cmp.get('v.profilesChartData').profilesValuesMap)
            .length !== 0
        ) {
          helper.setChartConfig(cmp, evt, helper);
