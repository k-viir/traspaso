...
({
  doInit: function(cmp, evt, helper) {
    let action = cmp.get('c.getGeographyChartData');
    let randomNumber = Math.floor(Math.random() * 100000);
    cmp.set('v.canvasId', 'geographyChart' + randomNumber);
    ...
    secondAction.setParams({ whatReport: 'geography' }); // if we want go to functionality change this as well
  ...
  handleFilters: function(cmp, evt, helper) {
    ...
    action.setCallback(this, function(response) {
      ...
      if (state === 'SUCCESS') {
        ...
        if (
          Object.keys(cmp.get('v.geographyChartData').geographyValuesMap)
            .length !== 0
        ) {
          helper.setChartConfig(cmp, evt, helper);
