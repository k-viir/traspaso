...
({
  setChartConfig: function(cmp, evt, helper) {
    let segmentationChartData = cmp.get('v.segmentationChartData');
    let formattedData = [];
    let blues = [];
    ...
    let segmentationLabels = Object.keys(
      cmp.get('v.segmentationChartData').segmentationValuesMap
    );
    cmp.set('v.segmentationLabels', segmentationLabels);
    segmentationLabels.forEach((elem) => {
      formattedData.push(segmentationChartData.segmentationValuesMap[elem]);
    });
          ...
          options: {
            onClick: (event, activeItems, chart) => {
              ...
              if (firstPoint) {
                helper.goToReport(
                  ...
                  evt,
                  helper,
                  cmp.get('v.segmentationLabels')[firstPoint.index]
                );
              }
            },
            responsive: true,
            ...
            plugins: {
              ...
              title: {
                display: true,
                text:
                  $A.get('$Label.c.Segmentation_chart_title') +
                  ' ' +
  ...
  goToReport: function(cmp, evt, helper, segmentationFilter) {
    let navService = cmp.find('navService');
    let reportFilters;

    reportFilters = {
      fv1: cmp.get('v.recordId'),
      fv2: segmentationFilter, // profile
